...
create table t_feedback (
   ...
   fb_type              character varying(50) not null,
   fb_location          character varying(300) not null,
   imgurl               character varying(200) not null,
   describes            character varying(500) not null,
   fb_time              timestamp            null default now(),
   fb_status            integer              null default '0',
   constraint t_feedback_pkey primary key (id)
);
...
create table t_goods (
   ...
   collect              integer              null default '0',
   talk                 integer              null default '0',
...
comment on column t_goods.collect is
'收藏数';

comment on column t_goods.talk is
'评论数';
...
comment on table t_goods_like is
'商品被用户点赞的数据';
...
comment on table t_user_like is
'用户点赞用户数据';
...
CREATE OR REPLACE VIEW public.v_mymessage AS
 SELECT u2.id as uid,
   ...
   FROM t_message m,
    t_user u1,
    t_user u2
  WHERE m.senderid::text = u1.id::text AND m.receiverid::text = u2.id::text AND m.state >= 0  ORDER BY m."time";
